    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const img_url = await fetch(
                `https://www.googleapis.com/youtube/v3/channels?part=snippet&id=${chanids.join(",")}&thumbnails&key=${key}`
            );
            const newimg = await img_url.json();


            const durations = await fetch(`${base}videos?id=${vidIds}&part=contentDetails&key=${key}`)
            ...
            const updateData = newdata.items?.map((ele) => {
                ...
                const channelimg = newimg.items?.find((el) => ele.snippet.channelId == el.id);
                if (channelimg) {
                            ...
                            <div className="vid-footer d-flex justify-content-between my-2">
                                <div className="main-desc">
                                    <div className="disc d-flex">
                                        ...
                                        <img src={ele.snippet.chanImg.default.url} style={{ width: "50px", height: "50px" }}
